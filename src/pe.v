 ...
 *    step (K=4); only the even activation of the pair is used. Half the
...
module pe (
    ...
    // 4 MAC steps: sparse max |acc| = 2048, dense max = 4096 —
    // both exact in a 14-bit signed accumulator (dense +4096 would
    // overflow 13 bits by exactly one count).
