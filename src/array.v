...
module array (
    ...
    input  wire [23:0]  b_in,     // 3 cols x Int7+1 weight byte
    output wire [107:0] data_out  // 9 accumulators x 12 bits, row-major
);

    // a_pipe[row][col]: pair flowing right; b_pipe[row][col]: byte flowing down
    wire [7:0]  a_pipe [0:2][0:3];
    wire [7:0]  b_pipe [0:3][0:2];
    wire [11:0] c_bus  [0:2][0:2];
    ...
    generate
        for (row = 0; row < 3; row = row + 1) begin : flat_row
            for (col = 0; col < 3; col = col + 1) begin : flat_col
                assign data_out[12*(row*3+col) +: 12] = c_bus[row][col];
            end
        end
    endgenerate
