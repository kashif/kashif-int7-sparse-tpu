/*
 * Weight memory: 3 columns x 4 Int7+1 bytes.
 *
 * Each byte {select, value[6:0]} covers TWO dense weight positions
 * (contraction steps k=2j and k=2j+1 of column c): value sits at
 * k = 2j + select, the other position is zero. 12 bytes encode a dense
 * 8x3 weight matrix — the 2x storage saving of the Int7+1 format.
 *
